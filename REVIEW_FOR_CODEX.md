...
검증한 내용과 발견한 문제" 참고). 그 과정에서 코드 버그 2개(모듈 파싱, 그리고
**reload 명령이 실패해도 종료 코드 0을 반환할 수 있는 침묵 실패** — 이게 가장
심각했음)와, Windows에서 **반드시 필요한 배포 전제 조건 2개**(Apache를
Windows 서비스로 등록해야 함, 그리고 reload 명령 자체를 `-k graceful`이 아닌
`-k restart`로 바꿔야 함)를 발견해 모두 수정/반영했습니다. 다만 이 검증은
여전히 Windows 11 개발 머신
...
8. **⚠️⚠️ 가장 중요한 버그: `-k restart`가 실패해도 종료 코드는 0을 반환할 수
   있음 (침묵 실패)** — Node 서버를 관리자 권한 없이 실행한 상태에서 적용을
   테스트했더니, 문법 검사 통과 → `httpd -k restart` 실행 → **종료 코드
   0, stdout/stderr 모두 빈 문자열** → 시스템이 "성공"으로 판단 →
   `configStatus: APPLIED`. 그런데 실제 도메인으로 접속해보니 여전히 이전
   상태였습니다 — Apache가 전혀 재시작되지 않은 것입니다. 원인은 서비스 제어
   권한 부족(AH10014)이었는데, **이 오류가 우리 프로세스의 stdout/stderr가
   아니라 Apache 자신의 `error.log` 파일에만 기록되어**, `execFile`로 캡처하는
   종료 코드/stderr만으로는 이 실패를 원천적으로 감지할 수 없었습니다. 이것은
   단순한 "느린 실패"가 아니라 **"성공이라고 거짓 보고하는" 실패**라서, 문서화만
   으로는 부족하다고 판단해 코드를 수정했습니다.

   **수정**: `ApacheStatusResult`에 실행 중인 모든 `httpd.exe` PID 배열
   (`pids`)을 추가하고, reload 명령 실행 **직전**과 **직후**에 각각
   `getStatus()`를 호출해 PID 집합을 비교합니다. 재시작 전에 Apache가 이미
   실행 중이었는데 재시작 후에도 정확히 같은 PID 집합이면 — 종료 코드가 0이고
   프로세스가 "실행 중"이더라도 — 실제로는 아무것도 재시작되지 않은 것으로
   판단해 자동 롤백합니다 (`apps/server/src/services/apache/
   ApacheApplyService.ts`의 `samePidSet`). `pids`를 제공하지 않는 러너(기존
   Mock 기본값 등)에는 이 추가 검사가 전혀 적용되지 않도록 해 기존 동작을
   깨지 않았습니다. 회귀 테스트 2개 추가 (성공 시 오탐 없음 확인 포함).
   수정 후 **같은 비-관리자 환경에서 동일하게 재현해보니 이번엔 정확히
   `success: false` + 자동 롤백 + "실제로 재시작되지 않아..." 메시지**로
   바뀐 것을 확인했습니다.

   **시사점**: 관리자 권한 없이 Node 프로세스를 실행하면 이 안전장치 덕분에
   최소한 "거짓 성공"은 막을 수 있지만, 애초에 관리자 권한(DEPLOY_WINDOWS_
   SERVER.md 9절)이 없으면 **어떤 적용도 실제로는 성공할 수 없습니다.**

이 검증에 사용한 httpd.conf 변경(모듈 2개 활성화, Listen 포트 변경, 서비스 등록)은
이 개발 머신에만 적용된 것이며, 저장소에는 커밋되지 않습니다 (Apache 설치
자체는 이 프로젝트의 일부가 아님). 코드 변경(모듈 파싱 수정, `-k restart`로
전환, PID 집합 비교를 통한 침묵 실패 감지)만 커밋되었습니다.
...
5. **(수정 완료, 회귀 테스트 있음) `-k restart`가 실패해도 종료 코드 0을 반환할
   수 있는 침묵 실패**: 위 "실제 Apache로 검증한 내용과 발견한 문제" 8번 참고.
   `ApacheApplyService`가 reload 전/후 httpd.exe PID 집합을 비교해 감지하도록
   수정함. 운영 서버(정상적으로 관리자 권한 실행)에서도 이 검사가 오탐 없이
   동작하는지 확인 권장.
6. **npm audit 경고**: 초기 `npm install` 시 5~7건의 취약점 경고가 표시됩니다
...
7. **린트 미실행**: ESLint 설정 파일은 추가했지만 `npm run lint`를 전체
   실행/정리하지는 않았습니다.
...
- [ ] **PID 집합 비교 기반 침묵 실패 감지 로직**(`ApacheApplyService.
      samePidSet`) 검토 — Node 프로세스가 서비스 제어 권한이 없을 때
      `-k restart`가 종료 코드 0을 반환하며 실제로는 아무것도 하지 않는 것을
      막기 위해 추가함 (알려진 문제 5 / "실제 Apache로 검증한 내용" 8번 참고).
      운영 서버(관리자 권한으로 정상 실행되는 환경)에서도 이 검사가 정상 동작을
      오탐하지 않는지 확인 필요
